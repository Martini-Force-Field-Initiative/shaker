...
def measure_bonded_terms(
    ...
    for resid in tqdm(resids, desc="Measuring Bonded parameters"):
        ...
        angs = [
            _beadstoangle(u, resid, resname, a, resid, resname, b, resid, resname, c)
...
def _beadstodihedral(
    ...
    ag = u.select_atoms(A) + u.select_atoms(B) + u.select_atoms(C) + u.select_atoms(D)
    return ag.dihedral


###Define function to get the angle object for a specified set of 3 beads
def _beadstoangle(
    u, resid1, resname1, name1, resid2, resname2, name2, resid3, resname3, name3
):
    selection = "resname {} and resid {} and name {}"
    A = selection.format(resname1, resid1, name1)
    B = selection.format(resname2, resid2, name2)
...
def _beadstodistance(u, resid1, resname1, name1, resid2, resname2, name2):
    ...
    if len(A) != 1 or len(B) != 1:
        raise ValueError(
            f"Expected 1 atom each, got len(A)={len(A)}, len(B)={len(B)} "
